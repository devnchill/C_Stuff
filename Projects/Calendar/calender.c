...
// Function to find the day of the week for a given date using Zeller's Congruence
int findStartingDay(int year, int month) {
    ...
    int h = (1 + (int)floor(13 * (double)(month + 1) / 5) + K + (int)floor((double)K / 4) + (int)floor((double)J / 4) - 2 * J) % 7;
    
    // Adjust h to match our days array (0 = Sunday, 1 = Monday, ..., 6 = Saturday)
    if (h < 0)
...
int main(void){
    ...
    for (int month = 0; month < 12; month++) {
        ...
        int start_day = findStartingDay(input_year, month + 1); // month + 1 because Zeller's formula expects March = 3, April = 4, ..., January = 13, February = 14
